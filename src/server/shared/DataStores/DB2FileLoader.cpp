...
DB2FieldMeta::DB2FieldMeta(bool isSigned, DB2Former type, char const* name)
...
uint32 DB2FileLoadInfo::GetStringFieldCount(bool localizedOnly) const
{
    ...
    for (char fieldType : TypesString)
        if (fieldType == FT2_STRING || (fieldType == FT2_STRING_NOT_LOCALIZED && !localizedOnly))
            ++stringFields;
                ...
                switch (_loadInfo->TypesString[fieldIndex])
                {
                    ...
                    case FT2_STRING_NOT_LOCALIZED:
                    {
                        ...
                        *slot = (char const**)(&stringHoldersPool[stringHoldersRecordPoolSize * y + stringFieldOffset]);
                        if (_loadInfo->TypesString[fieldIndex] == FT2_STRING)
                    ...
                    default:
                        ASSERT_FORMAT(false, "Unknown format character '%c' found in %s meta for field %s",
        ...
        switch (_loadInfo->Meta->Fields[_loadInfo->Meta->ParentIndexField].Type)
        {
            ...
            case FT2_INT:
                *reinterpret_cast<uint32*>(&recordData[parentIdOffset]) = parentId;
                break;
            default:
                ASSERT_FORMAT(false, "Unhandled parent id type '%c' found in %s", _loadInfo->Meta->Fields[_loadInfo->Meta->ParentIndexField].Type, _fileName);
                break;
        }
    }
}
...
T DB2FileLoaderRegularImpl::RecordGetVarInt(uint8 const* record, uint32 field, uint32 arrayIndex) const
{
    ...
    switch (compressionType)
    {
        ...
        default:
            ASSERT_FORMAT(false, "Unhandled compression type %u in %s", uint32(_columnMeta[field].CompressionType), _fileName);
            break;
    }
...
bool DB2FileLoaderSparseImpl::LoadTableData(DB2FileSource* source, uint32 section)
{
    ...
    _data = std::make_unique<uint8[]>(_sections[0].CatalogDataOffset - _dataStart);
        ...
        for (uint32 arr = 0; arr < _loadInfo->Meta->Fields[field].ArraySize; ++arr)
        {
            ...
            switch (_loadInfo->Meta->Fields[field].Type)
            {
                ...
                case FT2_STRING_NOT_LOCALIZED:
                    ...
                    break;
                default:
                    ASSERT_FORMAT(false, "Unknown format character '%c' found in %s meta", _loadInfo->Meta->Fields[field].Type, _fileName);
                    break;
            }
            ++combinedField;
        }
...
uint8 DB2Record::GetUInt8(char const* fieldName) const
{
    ...
    ASSERT_FORMAT(fieldIndex.first != -1, "Field with name %s does not exist!", fieldName);
...
void DB2FileLoader::Load(DB2FileSource* source, DB2FileLoadInfo const* loadInfo)
{
    ...
    if (_header.Signature != 0x32434457 && _header.Signature != 0x434C5331)
        throw DB2FileLoadException(Trinity::StringFormat("Incorrect file signature in %s, expected 'WDC2'/'1SLC', got %c%c%c%c", source->GetFileName(),
    ...
    for (uint32 i = 0; i < _header.SectionCount; ++i)
    {
        ...
        if (!(_header.Flags & 0x1) && section.CopyTableSize)
        {
    ...
    if (!loadInfo->Meta->HasIndexFieldInData())
    {
        ASSERT_FORMAT(!loadInfo->Fields[0].IsSigned, "ID must be unsigned");
        ++fieldIndex;
    }
    for (uint32 f = 0; f < loadInfo->Meta->FieldCount; ++f)
    {
        if (loadInfo->Fields[fieldIndex].IsSigned != _impl->IsSignedField(f))
            printf("Field % s in % s must be % s\n", loadInfo->Fields[fieldIndex].Name, source->GetFileName(), _impl->IsSignedField(f) ? "signed" : "unsigned");
